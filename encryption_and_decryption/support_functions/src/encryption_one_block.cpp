...
uint64_t encryption_one_block(const uint64_t& original_block, const std::vector<uint32_t>& generated_keys) {
	...
	for(int i{0}; i < 31; i++) {
		...
		G_converted_32_bits = G_conversion(first_32_bits, second_32_bits, generated_keys[i]); // применение G преобразования
		first_32_bits = second_32_bits;
		second_32_bits = G_converted_32_bits;
	}
	first_32_bits = G_conversion(first_32_bits, second_32_bits, generated_keys[31]); // самая последняя итерация(то есть 32ая)
	result = (static_cast<uint64_t>(first_32_bits) << 32) + (static_cast<uint64_t>(second_32_bits)); // результирующий блок
	return result;
}